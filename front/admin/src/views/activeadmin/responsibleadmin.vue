...
<template>
  <div class="bg-white rounded-lg shadow-sm p-6">
    <a-page-header title="活动申请管理" class="p-0 mb-6" :style="{ borderBottom: '1px solid #f0f0f0' }" />

    <a-empty v-if="activeList.length === 0" description="暂无需要申请的活动"
    ...
    <div v-else class="flex flex-col gap-4">
      <div v-for="item in activeList" :key="item.id"
        ...
        <div class="flex flex-col md:flex-row items-stretch gap-2 w-full md:w-auto">
          <a-button type="primary" size="small" :disabled="!(item.activitStatus == 0 || item.activitStatus == 6)"
            @click="requestReview(item.id)" class="!h-8">
          ...
          <a-button type="link" size="small" @click="router.push(`/activeadmin/${item.id}`)" class="!h-8">
