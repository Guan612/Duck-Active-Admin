...
export default function useActiveDetail() {
  ...
  const getActiveDetail = async () => {
    const res = await getActiveDetailAPI(detailId as string);
    setActiveDetail(res);
  };
  ...
  const isJoinActive = async () => {
    const res = await isJoinAPI(detailId as string);
    setJoinStatus(res);
  };

  const checkActiveStatus = async () => {
    const res = await checkActiveStatusAPI(detailId as string);
    return res;
  };
